...
void 0 !== topnav &&
  ...
  document.addEventListener("scroll", () => {
    window.scrollY > 99
      ...
      topnav.classList.contains("scrolled-shadow") &&
        (window.scrollY > 100
...
if (void 0 !== scrollTop && null != scrollTop) {
  ...
  window.addEventListener("load", d),
    ...
    scrollTop.addEventListener("click", g);
}
...
function hideDummySpinnerHeaderFooter(tablewrapperid) {
  ...
  let dummyFooter = document.querySelector(
    "#" + tablewrapperid + " .dummyfooter"
  );
...
if (void 0 !== bgimg && null != bgimg)
  for (
    var element, dataimgs = document.querySelectorAll("[data-img]"), i = 0;
    (element = dataimgs[i]);
    i++
  ) {
    ...
    (element.style.background = "url(" + h + ")"),
      void 0 !== a && null != a
        ? (element.style.backgroundPosition = a)
        : (element.style.backgroundPosition = "center center"),
      void 0 !== b && null != b
        ? (element.style.backgroundAttachment = b)
    ...
    for (const option of document.querySelectorAll(".custom-option")) {
      option.addEventListener("click", function () {
        ...
        if (void 0 !== input && null != input) {
          this.closest(".select").querySelector("input").value =
...
function resetDropDownList(elementId) {
  var ddlOptions = document.querySelector(
    "#" + elementId + " .select-wrapper .custom-option"
  );
  if (void 0 !== ddlOptions && null != ddlOptions) {
    for (const option of document.querySelectorAll(
      ...
      option
        ...
        pleaseSelectNode.textContent;
      var input = option.closest(".select").querySelector("input");
      if (void 0 !== input && null != input) {
        option.closest(".select").querySelector("input").value =
          pleaseSelectNode.getAttribute("data-value");
      }
    }
  }
}
...
function setNewOptionsForDdl(elementId, text, value) {
  $(`#${elementId} .custom-options`).append(
    `<span class='custom-option' data-text='${text}' data-value='${value}' value='${value}'>${text}</span>`
  );
  for (const option of document.querySelectorAll(
    `#${elementId} .custom-options .custom-option`
  )) {
    option.addEventListener("click", function () {
      this.parentNode
        .querySelector(".custom-option.selected")
...
function convertToLocalDatetimeIsoString(isoUtcString) {
  ...
  const dateTimeLocal = new Date(
    dateTimeUtc.getTime() - dateTimeUtc.getTimezoneOffset() * 60 * 1000
  );
  const formattedDateTime =
    dateTimeLocal.getFullYear() +
    "-" +
    (dateTimeLocal.getMonth() + 1).toString().padStart(2, "0") +
    "-" +
    dateTimeLocal.getDate().toString().padStart(2, "0") +
    "T" +
...
if (dtInput) {
  for (const datetimeInput of document.querySelectorAll(
    "input.datetimetext-input"
  )) {
    let datetime = new Date(datetimeInput.value);
    let clientMachineDateTime = new Date(
...
if (void 0 !== multichoice && null != multichoice)
  for (
    var element,
      multichoices = document.querySelectorAll("select.multichoice"),
      i = 0;
    (element = multichoices[i]);
    i++
  ) {
...
function getNumberAbbreviation(a) {
  ...
  if (a >= 1e3) {
    for (
      ...
      !(
        (
          (b = parseFloat((0 != c ? a / Math.pow(1e3, c) : a).toPrecision(d))) +
          ""
        ).replace(/[^a-zA-Z 0-9]+/g, "").length <= 2
      );
      d--
    );
...
function copyToClipboard(b, c) {
  ...
  (a.value = d),
    ...
    new bootstrap.Modal(document.getElementById(c), {}).show();
}
